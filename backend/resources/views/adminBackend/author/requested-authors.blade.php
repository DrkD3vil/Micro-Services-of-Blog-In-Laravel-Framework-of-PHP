...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="glass-card rounded-xl overflow-hidden fade-in-item" style="animation-delay: 0.4s">
        <div class="p-6 border-b border-border-color">
            <div class="flex flex-col md:flex-row md:items-center justify-between">
                ...
                <div class="flex space-x-3">
                    <div class="relative">
                        <input type="text" placeholder="Search requests..." class="bg-bg-secondary border border-border-color rounded-lg pl-10 pr-4 py-2 w-full md:w-64 focus:outline-none focus:ring-2 focus:ring-accent-color focus:border-transparent">
                ...
                <table class="w-full">
                    ...
                    <tbody>
                        ...
                        <tr class="table-row border-b border-border-color last:border-0">
                            ...
                            <td class="py-4 px-6">
                                <div class="flex items-center">
                                    <div class="h-10 w-10 rounded-full bg-bg-tertiary flex items-center justify-center mr-3">
                                        <span class="font-medium text-primary">{{ substr($user->name, 0, 1) }}</span>
                                    </div>
                                    <div>
                                        <p class="font-medium">{{ $user->name }}</p>
                                        <p class="text-sm text-muted">Applied {{ \Carbon\Carbon::parse($user->created_at)->diffForHumans() }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="py-4 px-6">
                            ...
                            <td class="py-4 px-6">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-warning/20 text-warning">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    {{ $user->author_status }}
                            ...
                            <td class="py-4 px-6">
                                <div class="flex space-x-2">
                                    <form action="{{ url('/approve-author/'.$user->id) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="btn-primary flex items-center space-x-1" title="Approve Author">
                                    ...
                                    <button class="p-2 rounded-lg border border-border-color hover:bg-bg-tertiary transition-colors" title="View Details">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-info" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
                                    </button>
                                    <button class="p-2 rounded-lg border border-border-color hover:bg-bg-tertiary transition-colors" title="Reject Request">
            ...
            <div class="md:hidden">
                ...
                <div class="border-b border-border-color last:border-0 p-6">
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-sm text-primary">{{ \Carbon\Carbon::parse($user->created_at)->diffForHumans() }}</p>
                    </div>
                    <div class="flex justify-between space-x-2">
                        <form action="{{ url('/approve-author/'.$user->id) }}" method="POST" class="flex-1">
                            @csrf
                            <button type="submit" class="btn-primary w-full flex items-center justify-center space-x-1">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                </svg>
                                <span>Approve</span>
                        ...
                        <button class="p-2 rounded-lg border border-border-color hover:bg-bg-tertiary transition-colors flex-1 flex items-center justify-center" title="View Details">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-info" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                            </svg>
                        </button>
                        <button class="p-2 rounded-lg border border-border-color hover:bg-bg-tertiary transition-colors flex-1 flex items-center justify-center" title="Reject Request">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-danger" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                            </svg>
                        </button>
        ...
        @else
            ...
            <div class="p-12 text-center">
                <div class="max-w-md mx-auto">
                    ...
                    <p class="text-muted mb-6">All author requests have been processed. New requests will appear here.</p>
                    <button class="btn-primary">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                        </svg>
        ...
        <div class="p-4 border-t border-border-color flex flex-col md:flex-row justify-between items-center">
            ...
            <div class="flex space-x-2">
                <button class="px-4 py-2 rounded-lg border border-border-color hover:bg-bg-tertiary transition-colors text-sm">
                    Previous
                </button>
                <button class="px-4 py-2 rounded-lg border border-border-color bg-accent-color text-white hover:bg-accent-hover transition-colors text-sm">
                    Next
                </button>
            </div>
        </div>
