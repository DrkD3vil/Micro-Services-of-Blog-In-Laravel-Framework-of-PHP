                            ...
                            <tr class="table-row border-b border-border-color last:border-0">
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex space-x-2">
                                        ...
                                        {{-- Rejection --}}
                                        <form action="{{ url('/reject-author/' . $user->id) }}" method="POST" class="inline">
    @csrf
    <button type="submit" class="p-2 rounded-lg  hover:bg-danger/20 hover:border-danger/30 transition-all duration-200 group" title="Reject Request" onclick="return confirm('Are you sure you want to reject {{ $user->name }}\'s author request?')">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-muted group-hover:text-danger transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="border-b border-border-color last:border-0 p-6">
                        ...
                        <div class="flex justify-end space-x-2">
                            ...
                            <form action="{{ url('/reject-author/' . $user->id) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit"
                                    class="p-2 rounded-lg border border-border-color hover:bg-danger/20 hover:border-danger/30 transition-all duration-200 group"
                                    title="Reject Request"
                                    onclick="return confirm('Are you sure you want to reject {{ $user->name }}\'s author request?')">
                                    <svg xmlns="http://www.w3.org/2000/svg"
                                        class="h-5 w-5 text-muted group-hover:text-danger transition-colors"
                                        fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
